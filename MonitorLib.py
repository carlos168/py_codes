...
def send_messages(iteminfo,reason,status):
    ...
    log_info("========= send_messages ========")
    log_info("=========sssdddddd====" + str(iteminfo[16]) + "=====" + str(iteminfo[20]) + "============")
    #根据通知对象获取通知用户
    if iteminfo[16]:
    ...
    log_info("=======================测试=======")
    log_info(str(mailset)+"===========mailset=====")
    if send_list['emaillist']:
            for i in send_list['emaillist']:
                log_info(str(mailset[0][1])+str(mailset[0][2])+str(mailset[0][3])+str(i)+str(subject)+str(mailContent))
